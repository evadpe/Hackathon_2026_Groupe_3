...
OUTPUT_DIR = BASE_DIR / "data" / "raw" / "bundles" / "incoherent"
...
def generate_base_business_data(supplier: pd.Series) -> dict:
...
def make_common_part(supplier: pd.Series, shared_data: dict) -> dict:
    return {
...
def recompute_amounts(doc_data: dict) -> dict:
    amount_ht = round(doc_data["quantity"] * doc_data["unit_price"], 2)
    amount_tva = round(amount_ht * 0.20, 2)
    amount_ttc = round(amount_ht + amount_tva, 2)

    doc_data["amount_ht"] = amount_ht
    doc_data["amount_tva"] = amount_tva
    doc_data["amount_ttc"] = amount_ttc
    return doc_data


def apply_incoherence_scenario(
    scenario: str,
    purchase_order_data: dict,
    quote_data: dict,
    invoice_data: dict,
    alt_supplier: pd.Series | None = None,
) -> tuple[dict, dict, dict, list[str]]:
    flags = []

    if scenario == "price_mismatch":
        invoice_data["unit_price"] = round(invoice_data["unit_price"] + random.uniform(1.0, 5.0), 2)
        invoice_data = recompute_amounts(invoice_data)
        flags.append("price_mismatch")

    elif scenario == "quantity_mismatch":
        invoice_data["quantity"] = invoice_data["quantity"] + random.randint(5, 30)
        invoice_data = recompute_amounts(invoice_data)
        flags.append("quantity_mismatch")

    elif scenario == "product_mismatch":
        current_product = invoice_data["product_name"]
        alternatives = [p for p in ALT_PRODUCTS if p != current_product]
        invoice_data["product_name"] = random.choice(alternatives)
        flags.append("product_mismatch")

    elif scenario == "supplier_mismatch":
        if alt_supplier is None:
            raise ValueError("alt_supplier est requis pour supplier_mismatch")

        invoice_data["supplier_company_name"] = alt_supplier["company_name"]
        invoice_data["supplier_address"] = alt_supplier["address"]
        invoice_data["supplier_postal_code"] = alt_supplier["postal_code"]
        invoice_data["supplier_city"] = alt_supplier["city"]
        invoice_data["supplier_country"] = alt_supplier["country"]
        invoice_data["supplier_siren"] = alt_supplier["siren"]
        invoice_data["supplier_siret"] = alt_supplier["siret"]
        invoice_data["supplier_vat_number"] = alt_supplier["vat_number"]
        flags.append("supplier_mismatch")

    return purchase_order_data, quote_data, invoice_data, flags
...
def generate_incoherent_bundle(bundle_index: int, supplier: pd.Series, all_suppliers: pd.DataFrame, scenario: str) -> None:
    shared_data = generate_base_business_data(supplier)
    common_part = make_common_part(supplier, shared_data)
    ...
    alt_supplier = None
    if scenario == "supplier_mismatch":
        alternatives = all_suppliers[all_suppliers["supplier_id"] != supplier["supplier_id"]]
        alt_supplier = alternatives.sample(1).iloc[0]

    purchase_order_data, quote_data, invoice_data, flags = apply_incoherence_scenario(
        scenario,
        purchase_order_data,
        quote_data,
        invoice_data,
    ...
    bundle_dir = OUTPUT_DIR / f"bundle_{bundle_index:03d}_{scenario}"
    bundle_dir.mkdir(parents=True, exist_ok=True)

    create_purchase_order_pdf(purchase_order_data, bundle_dir / "purchase_order.pdf")
    create_quote_pdf(quote_data, bundle_dir / "quote.pdf")
    create_invoice_pdf(invoice_data, bundle_dir / "invoice.pdf")

    metadata = {
        "bundle_id": f"bundle_{bundle_index:03d}",
        "scenario": scenario,
        "source_supplier_id": supplier["supplier_id"],
        "source_supplier_name": supplier["company_name"],
...
def main(n_bundles: int = 8) -> None:
    ...
    for i, (_, supplier) in enumerate(selected_suppliers.iterrows(), start=1):
        scenario = SCENARIOS[(i - 1) % len(SCENARIOS)]
        generate_incoherent_bundle(i, supplier, df, scenario)

    print(f"{n_bundles} bundles incohérents ont été générés dans :")
    print(OUTPUT_DIR)
