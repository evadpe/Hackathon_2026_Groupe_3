...
def generate_quote_data(supplier: pd.Series) -> dict:
...
def create_quote_pdf(quote_data: dict, output_path: Path) -> None:
    ...
    c.setFont("Helvetica", 11)
    c.drawString(50, height - 190, quote_data["company_name"])
    c.drawString(50, height - 210, quote_data["address"])
    c.drawString(50, height - 230, f"{quote_data['postal_code']} {quote_data['city']}")
    c.drawString(50, height - 250, f"SIREN : {quote_data['siren']}")
    c.drawString(50, height - 270, f"SIRET : {quote_data['siret']}")
    c.drawString(50, height - 290, f"TVA : {quote_data['vat_number']}")
...
def main() -> None:
    ...
    quote_data = generate_quote_data(supplier)

    output_path = DOCUMENTS_DIR / supplier_id / "quote.pdf"
    create_quote_pdf(quote_data, output_path)
